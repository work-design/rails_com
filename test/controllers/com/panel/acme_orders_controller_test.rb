...
class Com::Panel::AcmeOrdersControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'create ok' do
    assert_difference('AcmeOrder.count') do
      post panel_acme_orders_url, params: { #{singular_table_name}: { #{attributes_string} } }
    end
  ...
  test 'update ok' do
    patch panel_acme_order_url(@com_panel_acme_order), params: { #{singular_table_name}: { #{attributes_string} } }
    assert_response :success
  end
